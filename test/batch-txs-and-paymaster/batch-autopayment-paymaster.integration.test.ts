...
describe("Payments integration test using multicall and paymaster:", async () => {
  async function setup() {
    ...
    const erc20Contract = await deployContract(
      deployer,
      "ERC20Token",
      ["TOKEN", "TOKEN", 18],
    )
    ...
    const sponsoredPaymasterContract = await deployContract(
      deployer,
      "ApprovalPaymaster",
      [erc20ContractAddress]
    )

    const generalPaymasterContract = await deployContract(
      deployer,
      "GeneralPaymaster",
      [],
    )

    const sponsoredPaymasterContractAddress = sponsoredPaymasterContract.address;
    ...
    .sendTransaction({
      to: sponsoredPaymasterContractAddress,
      value: TRANSFER_AMOUNT,
    })
    .then((res) => res.wait());
