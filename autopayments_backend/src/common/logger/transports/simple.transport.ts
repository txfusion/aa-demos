...
import { LoggerSimpleFormatOptions } from 'common/logger/interfaces/format.interface';
...
export const simple = (options: LoggerSimpleFormatOptions = {}): winston.Logform.Format => {
  ...
  return winston.format.combine(
    ...
    winston.format.printf((log) => {
      ...
      return [timestamp, meta, `${level}:`, text, stack, extra].filter((v) => v !== null && v !== '').join(' ');
    }),
  );
};

export const simpleTransport = (...args: Parameters<typeof simple>): winston.transports.ConsoleTransportInstance => {
